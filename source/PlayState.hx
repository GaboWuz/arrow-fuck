...
class PlayState extends MusicBeatState
{
	...
	override public function create()
	{
		...
		var songNae:String = Paths.formatToSongPath(SONG.song);
	...
	override public function update(elapsed:Float)
	{
		...
		if (generatedMusic && PlayState.SONG.notes[Std.int(curStep / 16)] != null)
		{
			...
			if (camFollow.x != dad.getMidpoint().x + 150 && !PlayState.SONG.notes[Std.int(curStep / 16)].mustHitSection)
			{
				...
				// camFollow.setPosition(dad.getMidpoint().x + 150 + offsetX, dad.getMidpoint().y - 100 + offsetY); fuck u kade engine
		...
		if (generatedMusic)
			{
				...
				{	
					...
					if ((daNote.mustPress && daNote.tooLate && !FlxG.save.data.downscroll || daNote.mustPress && daNote.tooLate && FlxG.save.data.downscroll) && daNote.mustPress)
					{
							...
							if (daNote.noteFuck == 1 || daNote.noteFuck == 0)
								{
									health -= 0.075;
									vocals.volume = 0;
									if (theFunne)
	...
	*/
	...
	override function beatHit()
	{
		...
		if (curSong == 'Tutorial' && dad.curCharacter == 'kevin') {
